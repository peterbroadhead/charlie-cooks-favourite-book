  ...
  <>
    <header className="bg-sky-blue text-slate container mx-auto">
      <ul className="flex flex-wrap md:flex-nowrap"> 
        <li className="basis-full"><Link to="/"><h1 className="text-xl p-10 pl-0 text-center md:text-leftfont-axel hover:text-orange ease duration-700">{siteTitle}</h1></Link></li>               
        {menuItems.map((item) => (          
            <li className="flex-grow text-center md:text-center md:flex "><Link className="text-xl pr-10 md:p-10 hover:text-orange font-serif" to={item.path}>{item.title}</Link></li>          
        ))}
